...
top_by_class <- freq_all %>%
  dplyr::group_by(sentiment) %>%
  dplyr::slice_max(n, n = 50, with_ties = FALSE) %>%
  dplyr::ungroup()

pos_words <- top_by_class %>% dplyr::filter(sentiment == "positive") %>% dplyr::pull(splitted)
neg_words <- top_by_class %>% dplyr::filter(sentiment == "negative") %>% dplyr::pull(splitted)
neu_words <- top_by_class %>% dplyr::filter(sentiment == "neutral") %>% dplyr::pull(splitted)
...
naiveBayes <- setRefClass("naiveBayes",
       ...
       methods = list(
                    ...
                    #   message_words <- message_df %>% unnest_tokens(word, text, token="words") %>%
                    #                       filter(!word %in% splitted_stop_words)
                    #   message_words = message_words$word[message_words$word %in% rownames(word_probabilities)] # Make vector with words and drop unseen during training words
                    #   if (length(message_words) == 0) {
                    #     return ("neutral")
                    #   }
                    #   message_probability_by_class = apply(word_probabilities[message_words, ,drop=FALSE], 2, prod)
                    #   total_message_probability = sum(message_probability_by_class*class_probabilities)
                    #   prediction_by_class = (message_probability_by_class/total_message_probability)*class_probabilities
                    #   prediction = names(prediction_by_class)[which.max(prediction_by_class)]
                    # 
                    #   return (prediction)
                    # },
...
predictions <- sapply(test$text, function(msg) model$predict(msg))
failures <- test[predictions != test$sentiment, c("text", "sentiment")]
failures$predictions <- predictions[predictions != test$sentiment]
...
predictions <- sapply(test_balanced$text, function(msg) model$predict(msg))
failures <- test_balanced[predictions != test_balanced$sentiment, c("text", "sentiment")]
failures$predictions <- predictions[predictions != test_balanced$sentiment]
...
### Data preprocesing
  - Loading and cleaning – The raw CSV files are read into R, and punctuation or other unnecessary symbols are removed to reduce noise.
  ...
  - Stop-word removal – Common words that do not carry any meaning for classification are filtered out using a predefined stop-word list.
...
**Limitations:**
- Does not capture word order or context in sentences.
- Sensitive to class imbalance in the training data.
- Assumes conditional independence of words, which is rarely true in practice since the occurrence of one word often affects the probability of others.
...
### Sets imbalance
I checked the number of messages in each class for both the training and test sets and saw that the distributions were very different. Some classes had many more examples than others, which caused the model to misclassify minority classes. To fix this, I created a balanced split that preserved the class proportions in train and test. After doing this, the model’s predictions became more accurate, and metrics like F1, precision, and recall improved across all classes.
