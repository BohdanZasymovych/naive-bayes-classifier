...
naiveBayes <- setRefClass("naiveBayes",
       ...
       fields = list(
                    ...
                    # Alpha parameter for laplace smothing
                    alpha = "numeric"
                    ),
       ...
       methods = list(
                    ...
                    fit = function(X) {
                      matr = preprocess(X) + alpha
                      total_words_per_class <- colSums(matr)
                      word_probabilities <<- sweep(matr, 2, total_words_per_class, FUN="/")
                      
                      class_probabilities <<- table(X$sentiment) / nrow(X)
                    },
...
naiveBayes <- setRefClass("naiveBayes",
      ...
      preprocess = function(data) {
                    ...
                    # X is tidy_text, dataframe where each row is a word and column sentiment is class, column splitted is a word
    fit = function(X) {
                      matr = preprocess(X) + alpha # Apply Laplace smoothing
                      total_words_per_class <- colSums(matr)
                      word_probabilities <<- sweep(matr, 2, total_words_per_class, FUN="/")
...
# plot with precision, recall, and f1 for each class
